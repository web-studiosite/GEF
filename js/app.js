...
import { syncSession, getCurrentUser, logoutUser, getDefaultRouteForRole, isRouteAuthorized } from './auth.js';
...
/**
 * Inicialização do Sistema GEF
 */
export async function bootstrapGEF() {
  console.log('[GEF] Inicializando GEF Enterprise...');

  // 1. Inicializar cliente Supabase com credenciais salvas
  const creds = getCredentials();
  initSupabase(creds.url, creds.key);

  // 2. Sincronizar sessão de autenticação ativa e RBAC
  await syncSession();

  // 3. Montar a casca visual (Shell) e registrar ouvinte de rotas
  setupShell();
